...
def save_channel_to_excel(channel_id):
    ...
    if channel_id in ALLOWED_CHANNELS:
        return False  
    print(f"Adding Channel ID: {channel_id} to allowed list.")
...
async def add_channel(event):
    ...
    if chat_id != ALLOWED_ADMIN_GROUP :
        ...
        log_data.clear()
        os.remove(VC_Log)
        await bot.send_message(chat_id=chat_id, text=f"❌ This command can only be used in the allowed group.")
        return
    ...
    if not channel_username:
        await bot.send_message(chat_id=ALLOWED_ADMIN_GROUP, text=f"❌ Invalid channel link. Example:\n`/addchannel https://t.me/example_channel`")
    ...
    try:
        ...
        if save_channel_to_excel(channel_id):
            await bot.send_message(chat_id=ALLOWED_ADMIN_GROUP, text=f"✅ Channel {channel_username} (ID: {channel_id}) added successfully!")
            if os.path.exists(EXCEL_FILE):
                await bot.send_document(chat_id=ALLOWED_ADMIN_GROUP, document=open(EXCEL_FILE, 'rb'),
                                        caption="📄 Updated Allowed Channels List")
            else:
                await bot.send_message(chat_id=ALLOWED_ADMIN_GROUP, text="❌ The updated channel list is missing.")

        else:
            await bot.send_message(chat_id=ALLOWED_ADMIN_GROUP, text=f"⚠️ Channel {channel_username} is already in the list.")
...
async def restart_bot(event):
    ...
    await bot.send_message(chat_id=ALLOWED_ADMIN_GROUP, text=f"🔄 Restarting bot...")
    try:
        ...
        monitor_task = asyncio.create_task(monitor_vc_and_ban())  
        await bot.send_message(chat_id=ALLOWED_ADMIN_GROUP, text=f"🔄 Bot restarted and monitoring resumed.")
...
async def monitor_vc_and_ban():
    ...
    async with client:
        ...
        full_group_id = int(f"-100{group.id}")
                ...
                for p in participants:
                    if hasattr(p.peer, 'channel_id'):
                        ...
                        if channel_id in ALLOWED_CHANNELS:
                            ...
                            try:
                                print("Trying bot ban...")
                                
                                await bot.ban_chat_sender_chat(
    chat_id=full_group_id,
    sender_chat_id=full_channel_id
)
                                print("✅ BOT SUCCESSFULLY BANNED CHANNEL")

                            except Exception as e:
                                print("❌ BOT BAN FAILED:", e)
                        ...
                        if (p.video or p.presentation):
                            ...
                            await bot.send_message(chat_id=ALLOWED_ADMIN_GROUP, text=f"⚠️ Banned For Sharing Screen or Camera {user_name} {user_id}")
